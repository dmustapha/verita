...
|---|---|---|
| Unbacked attestation (post a price with nothing at risk) | stake lock in `attest` (first statement checks `stakeOf[msg.sender] >= minStake`; single writer of `attestations[asset]`) | `src/Verita.sol` |
...
| Killshot report precision loss | BigInt coercion of price / timestampNs / nonce in the route | `web/app/api/killshot/route.ts` |
