...
| CL-3 | A wrongful liquidation on a bad mark is refused on 196 | the refuse tx hash in `submission/proof.md`; `cast tx $HASH` shows the revert reason `MarketNotOpen`/`Diverged` | build-gate (HERO) |
