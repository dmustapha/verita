...
export async function sourceChainlink(_feedId: string): Promise<Quote | null> {
...
export async function sourceYahoo(symbol: string): Promise<Quote | null> {
  // Fail CLOSED: Yahoo rate-limits (429) or omits regularMarketPrice routinely. Return null on any
  // malformed/absent price rather than throwing (crashes the run) or emitting NaN (attests garbage).
  ...
  return { price, status: 2 /*REGULAR — operator/relayer-tagged*/, ts: Date.now() };
}
